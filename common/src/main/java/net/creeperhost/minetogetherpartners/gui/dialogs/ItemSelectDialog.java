package net.creeperhost.minetogetherpartners.gui.dialogs;
...
public class ItemSelectDialog<E> extends GuiElement<ItemSelectDialog<E>> implements BackgroundRender {
    ...
    public ItemSelectDialog(@NotNull GuiParent<?> parent, Component title, List<E> items, E defaultItem) {
        this(parent, title, items, defaultItem, e -> Component.literal(String.valueOf(e)));
    }
    ...
    public ItemSelectDialog(@NotNull GuiParent<?> parent, Component title, List<E> items, E defaultItem, Function<E, Component> toComponentFunction) {
        ...
        GuiText titleText = new GuiText(this, title)
                ...
                .constrain(HEIGHT, dynamic(() -> (double) font().wordWrapHeight(title, (int) xSize() - 10)));

        //Buttons
        GuiButton select = button(this, Component.translatable("minetogether:gui.select_dialog.select"))
        ...
        GuiButton cancel = button(this, Component.translatable("minetogether:gui.select_dialog.cancel"))
                .onPress(this::close)
        ...
        this.searchField = new GuiTextField(searchBg)
                ...
                .setSuggestion(Component.translatable("minetogether:gui.select_dialog.search"));
...
//                .fill(0xA0202020)
        ...
        itemList.setDisplayBuilder((list, item) -> {
            ...
            button.constrain(HEIGHT, dynamic(() -> (double)font().wordWrapHeight(text, (int)button.xSize() - 4) + 2));
