...
import static net.creeperhost.minetogetherpartners.MineTogetherPartners.WEB_ENGINE;
...
public class OrderRequests {
    ...
    public static List<GetDataCentresRequest.DC> getDataCenters(int ram) {
        try {
            GetDataCentresRequest.Response response = MineTogetherPartners.API.execute(new GetDataCentresRequest(ram)).apiResponse();
    ...
    public static Map<String, Integer> getLocations() {
        try {
            GetLocationsRequest.Response response = MineTogetherPartners.API.execute(new GetLocationsRequest()).apiResponse();
    ...
    public static GetClosestDCRequest.Response getDCsByDistance() {
        try {
            return MineTogetherPartners.API.execute(new GetClosestDCRequest()).apiResponse();
        } catch (Throwable e) {
    ...
    public static int getDCLatency(String latencyUrl, long distance) {
        try {
            GetLatencyRequest.Response response = MineTogetherPartners.API.execute(new GetLatencyRequest(latencyUrl)).apiResponse();
            if (!response.getStatus().equals("success")) {
    ...
    public static OrderSummary getSummary(Order order, String promo) {
        ...
        try {
            var recResponse = MineTogetherPartners.API.execute(new GetRecommendRequest(version, order.playerAmount)).apiResponse();
            ...
            var summary = MineTogetherPartners.API.execute(new GetSummaryRequest(order.country, recommended)).apiResponse();
            ...
            var currencyResponse = MineTogetherPartners.API.execute(new GetCurrencyRequest(order.country)).apiResponse();
            String prefix = currencyResponse.prefix;
            String suffix = currencyResponse.suffix;
            String id = currencyResponse.id;

            var productResponse = MineTogetherPartners.API.execute(new GetProductRequest(recommended)).apiResponse();
    ...
    private static String getWebResponse(String urlString) throws IOException {
        EngineRequest request = WEB_ENGINE.newRequest()
                ...
                .header("Fingerprint", MineTogetherPartners.FINGERPRINT)
    ...
    public static ApiResponse getNameAvailable(String name) {
        try {
            return MineTogetherPartners.API.execute(new GetNameAvailableRequest(name)).apiResponse();
        } catch (Throwable e) {
            LOGGER.error("Failed to retrieve datacenter list", e);
    ...
    public static boolean doesAccountExist(String email) {
        try {
            ApiResponse response = MineTogetherPartners.API.execute(new PostEmailExistsRequest(email)).apiResponse();
    ...
    public static PostLoginRequest.Response doLogin(String email, String password) {
        try {
            PostLoginRequest.Response response = MineTogetherPartners.API.execute(new PostLoginRequest(email, password)).apiResponse();
            if (!response.getStatus().equals("success")) {
    ...
    public static PostOrderRequest.Response placeOrder(Order order, String dcId, String pregen, String fallbackName) {
        try {
            PostOrderRequest.Response response = MineTogetherPartners.API.execute(new PostOrderRequest(order, dcId, pregen, fallbackName)).apiResponse();
            if (!response.getStatus().equals("success")) {
                LOGGER.error("Failed to complete login. Api returned: {}", response.getMessageOrNull());
            }
            return response;
    ...
    public static PostCreateAccountRequest.Response createAccount(Order order) {
        try {
            PostCreateAccountRequest.Response response = MineTogetherPartners.API.execute(new PostCreateAccountRequest(order)).apiResponse();
            if (!response.getStatus().equals("success")) {
