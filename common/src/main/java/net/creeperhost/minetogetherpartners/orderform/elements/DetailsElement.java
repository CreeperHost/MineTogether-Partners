...
public class DetailsElement extends GuiElement<DetailsElement> {
    ...
    public DetailsElement(@NotNull GuiParent<?> parent, OrderGui gui) {
        ...
        GuiElement<?> lastElement = new GuiText(this, Component.translatable("minetogether:gui.order.details").withStyle(ChatFormatting.UNDERLINE, ChatFormatting.GOLD))
        ...
        GuiElement<?> password = passwordBox(match(get(LEFT)), centerLeft, TextState.create(() -> gui.order.password, val -> gui.order.password = val), Component.translatable("minetogether.info.password"))
                .constrain(TOP, relative(email.get(BOTTOM), 4));
        GuiElement<?> password2 = passwordBox(centerRight, match(get(RIGHT)), TextState.create(() -> confirmPassword, val -> confirmPassword = val), Component.translatable("minetogether.info.password_confirm"))
                .setEnabled(() -> !loginMode)
                .constrain(TOP, relative(email.get(BOTTOM), 4));

        GuiElement<?> firstName = detailsBox(match(get(LEFT)), centerLeft, TextState.create(() -> gui.order.firstName, val -> gui.order.firstName = val), Component.translatable("minetogether.info.first_name"))
                .setEnabled(() -> !loginMode)
                .constrain(TOP, relative(password.get(BOTTOM), 4));
        GuiElement<?> lastName = detailsBox(centerRight, match(get(RIGHT)), TextState.create(() -> gui.order.lastName, val -> gui.order.lastName = val), Component.translatable("minetogether.info.last_name"))
                .setEnabled(() -> !loginMode)
                .constrain(TOP, relative(password.get(BOTTOM), 4));

        GuiElement<?> address = detailsBox(match(get(LEFT)), centerLeft, TextState.create(() -> gui.order.address, val -> gui.order.address = val), Component.translatable("minetogether.info.address"))
                .setEnabled(() -> !loginMode)
                .constrain(TOP, relative(firstName.get(BOTTOM), 4));
        GuiElement<?> city = detailsBox(centerRight, match(get(RIGHT)), TextState.create(() -> gui.order.city, val -> gui.order.city = val), Component.translatable("minetogether.info.city"))
                .setEnabled(() -> !loginMode)
                .constrain(TOP, relative(firstName.get(BOTTOM), 4));

        GuiElement<?> zipCode = detailsBox(match(get(LEFT)), centerLeft, TextState.create(() -> gui.order.zip, val -> gui.order.zip = val), Component.translatable("minetogether.info.zip"))
                .setEnabled(() -> !loginMode)
                .constrain(TOP, relative(address.get(BOTTOM), 4));
        GuiElement<?> state = detailsBox(centerRight, match(get(RIGHT)), TextState.create(() -> gui.order.state, val -> gui.order.state = val), Component.translatable("minetogether.info.state"))
                .setEnabled(() -> !loginMode)
                .constrain(TOP, relative(address.get(BOTTOM), 4));

        Map<String, OrderGui.Country> countryMap = getCountries();
        GuiButton country = MTStyle.Flat.button(this, () -> Component.literal(gui.getSelectedCountry().toString()))
                .setEnabled(() -> !loginMode)
                .onPress(() -> new ItemSelectDialog<>(this.getModularGui().getRoot(), Component.translatable("minetogether:gui.order.select_country"), new ArrayList<>(countryMap.values()), countryMap.get(gui.order.country)).setOnItemSelected(item -> {
        ...
        GuiElement<?> phone = detailsBox(centerRight, match(get(RIGHT)), TextState.create(() -> gui.order.phone, val -> gui.order.phone = val), Component.translatable("minetogether.info.phone"))
                .setEnabled(() -> !loginMode)
                .constrain(TOP, relative(zipCode.get(BOTTOM), 4));

        GuiButton login = MTStyle.Flat.button(this, Component.translatable("minetogether:gui.button.login"))
                .setEnabled(() -> loginMode)
        ...
        GuiText loginInfo = new GuiText(this, Component.empty())
                .setEnabled(() -> loginMode)
                .setWrap(true)
                .setTextSupplier(() -> {
                    if (loggingIn) {
